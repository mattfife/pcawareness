...
//--------------------------------------------------------------------------------------
// Generated by ShaderGenerator.exe version 0.11
//--------------------------------------------------------------------------------------
...
#else
    texture2D diffuse < string Name = "diffuse"; string UIName = "diffuse"; string ResourceType = "2D";>;
    sampler2D SAMPLER0 = sampler_state{ texture = (diffuse);};
    texture2D normal < string Name = "normal"; string UIName = "normal"; string ResourceType = "2D";>;
    sampler2D SAMPLER1 = sampler_state{ texture = (normal);};
    texture2D specular < string Name = "specular"; string UIName = "specular"; string ResourceType = "2D";>;
    sampler2D SAMPLER2 = sampler_state{ texture = (specular);};
#endif

// -------------------------------------
float4 NMLAYER( PS_INPUT input )
{
    return ((
#ifdef _CPUT
normal.Sample( SAMPLER0, (((input.UV0)) *(float2(5, 5).xyyy)) )
#else
Tex2D( SAMPLER1, (((input.UV0)) *(float2(5, 5).xyyy)) )
#endif
) *(2)) -(1);
}

// -------------------------------------
float4 DMLAYER( PS_INPUT input )
{
    return 
#ifdef _CPUT
diffuse.Sample( SAMPLER0, (((input.UV0)) *(float2(5, 5).xyyy)) )
#else
Tex2D( SAMPLER0, (((input.UV0)) *(float2(5, 5).xyyy)) )
#endif
;
}

// -------------------------------------
float4 SMLAYER( PS_INPUT input )
{
    return 
#ifdef _CPUT
specular.Sample( SAMPLER0, (((input.UV0)) *(float2(5, 5).xyyy)) )
#else
Tex2D( SAMPLER2, (((input.UV0)) *(float2(5, 5).xyyy)) )
#endif
;
}
...
{
    return DMLAYER(input);
}

// -------------------------------------
float4 NORMAL( PS_INPUT input )
{
    return NMLAYER(input);
}

// -------------------------------------
float4 SPECULAR( PS_INPUT input )
{
    return SMLAYER(input);
}
