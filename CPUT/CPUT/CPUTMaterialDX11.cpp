...
void ReadMacrosFromConfigBlock(
    ...
    D3D_SHADER_MACRO **pUserSpecifiedMacros,
    int               *pNumUserSpecifiedMacros,
    D3D_SHADER_MACRO **pFinalShaderMacros
){
    ...
    *pFinalShaderMacros = new D3D_SHADER_MACRO[*pNumUserSpecifiedMacros + numPassedInMacros + 1];

    // Copy the passed-in macro pointers to the final array
    int jj;
    for( jj=0; jj<numPassedInMacros; jj++ )
    {
        (*pFinalShaderMacros)[jj] = *(D3D_SHADER_MACRO*)&pShaderMacros[jj];
    }

    // Create a D3D_SHADER_MACRO for each of the macros specified in the .mtl file.
    // And, add their pointers to the final array
    *pUserSpecifiedMacros = new D3D_SHADER_MACRO[*pNumUserSpecifiedMacros];
...
CPUTResult CPUTMaterialDX11::LoadMaterial(
    ...
    CPUTConfigEntry *pValue;
    D3D_SHADER_MACRO *pFinalShaderMacros;
    D3D_SHADER_MACRO *pUserSpecifiedMacros = NULL;
    ...
    if( mConfigBlock.GetValueByName( L"MultiMaterial" )->ValueAsBool() )
    {
        ...
        for( int ii=0; ii<mSubMaterialCount; ii++ )
        {
            ...
            if( pMacrosBlock )
            {
                ReadMacrosFromConfigBlock( pMacrosBlock, (D3D_SHADER_MACRO*)pShaderMacros, &pUserSpecifiedMacros, &numUserSpecifiedMacros, &pFinalShaderMacros );
            } else
            {
                pFinalShaderMacros = (D3D_SHADER_MACRO*)pShaderMacros;
            }

            mpSubMaterials[ii] = pAssetLibrary->GetMaterial( mpSubMaterialNames[ii], false, pModel, meshIndex, (const char**)pFinalShaderMacros );
    ...
    // If we allocated pFinalShaderMacros, then delete it
    if( pFinalShaderMacros != (D3D_SHADER_MACRO*)pShaderMacros )
    {
        SAFE_DELETE_ARRAY( pFinalShaderMacros );
        SAFE_DELETE_ARRAY( pUserSpecifiedMacros );
    }
