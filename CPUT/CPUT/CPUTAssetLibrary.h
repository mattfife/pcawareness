...
public:
    ...
    void SetMediaDirectoryName( const std::string &directoryName)
    {
        ...
        std::wstring localDirectoryName = s2ws(directoryName.c_str());
    ...
    void AddMaterial(        const cString &name, CPUTMaterial *pMaterial, const char **pShaderMacros=NULL, const CPUTModel *pModel=NULL, int meshIndex=-1){ AddAssetInstance( name, pMaterial, &mpMaterialList,       &mpMaterialListTail,       &mpInstancedMaterialList,       &mpInstancedMaterialListTail,       pShaderMacros, pModel, meshIndex ); }
    void AddTexture(         const cString &name, CPUTTexture  *pTexture,                                   const CPUTModel *pModel=NULL, int meshIndex=-1){ AddAssetInstance( name, pTexture,  &mpTextureList,        &mpTextureListTail,        &mpInstancedTextureList,        &mpInstancedTextureListTail,        NULL,          pModel, meshIndex ); }
    void AddBuffer(          const cString &name, CPUTBuffer   *pBuffer,                                    const CPUTModel *pModel=NULL, int meshIndex=-1){ AddAssetInstance( name, pBuffer,   &mpBufferList,         &mpBufferListTail,         &mpInstancedBufferList,         &mpInstancedBufferListTail,         NULL,          pModel, meshIndex ); }
    void AddConstantBuffer(  const cString &name, CPUTBuffer   *pBuffer,                                    const CPUTModel *pModel=NULL, int meshIndex=-1){ AddAssetInstance( name, pBuffer,   &mpConstantBufferList, &mpConstantBufferListTail, &mpInstancedConstantBufferList, &mpInstancedConstantBufferListTail, NULL,          pModel, meshIndex ); }
    ...
    {
        return (CPUTBuffer*)   FindAsset( name, pModel?mpInstancedBufferList:mpBufferList,                   nameIsFullPathAndFilename, NULL,          pModel, meshIndex ); 
    }
    ...
    {
        return (CPUTBuffer*)   FindAsset( name, pModel?mpInstancedConstantBufferList:mpConstantBufferList,   nameIsFullPathAndFilename, NULL,          pModel, meshIndex );
    }
    CPUTMaterial *FindMaterial(                const cString &name, bool nameIsFullPathAndFilename=false, const char **pShaderMacros=NULL, const CPUTModel *pModel=NULL, int meshIndex=-1)
    {
        return (CPUTMaterial*) FindAsset( name, pModel?mpInstancedMaterialList:mpMaterialList,               nameIsFullPathAndFilename, pShaderMacros, pModel, meshIndex );
    }
    ...
    CPUTMaterial         *GetMaterial(        const cString &name, bool nameIsFullPathAndFilename=false, const CPUTModel *pModel=NULL, int meshIndex=-1, const char **pShaderMacros=NULL, int numSystemMaterials=0, cString *pSystemMaterialNames=NULL );
...
protected:
    ...
    void AddAsset(         const cString &name, void *pAsset, CPUTAssetListEntry **pHead, CPUTAssetListEntry **pTail, const char **pShaderMacros=NULL, const CPUTModel *pModel=NULL, int meshIndex=-1 );
    void AddAssetInstance( const cString &name, void *pAsset, CPUTAssetListEntry **pHead, CPUTAssetListEntry **pTail, CPUTAssetListEntry **pInstanceHead, CPUTAssetListEntry **pInstanceTail, const char **pShaderMacros=NULL, const CPUTModel *pModel=NULL, int meshIndex=-1 );
