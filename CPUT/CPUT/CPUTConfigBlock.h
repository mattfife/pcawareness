...
public:
    ...
    static CPUTConfigEntryA  &sNullConfigValue;
    ...
    float ValueAsFloat(void)
    {
        float fValue=0;
        int retVal;
        retVal=sscanf_s(szValue.c_str(), "%g", &fValue ); // float (regular float, or E exponentially notated float)
        ASSERT(0!=retVal, _L("ValueAsFloat - value specified is not a float"));
        return fValue;
    }
    int ValueAsInt(void)
    {
        int nValue=0;
        int retVal;
        retVal=sscanf_s(szValue.c_str(), "%d", &nValue ); // signed int (NON-hex)
        ASSERT(0!=retVal, _L("ValueAsInt - value specified is not a signed int"));
        return nValue;
    }
    UINT ValueAsUint(void)
    {
        UINT nValue=0;
        int retVal;
        retVal=sscanf_s(szValue.c_str(), "%u", &nValue ); // unsigned int
        ASSERT(0!=retVal, _L("ValueAsUint - value specified is not a UINT"));
        return nValue;
    }
    bool ValueAsBool(void)
