...
//-----------------------------------------------------------------------------
    ...
    #define ASSERTA(Condition, Message) { if( !(Condition) ) { std::string msg = std::string(__FUNCTION__) + ":  " + Message; OutputDebugStringA(msg.c_str()); DEBUGMESSAGEBOX("Assert", msg ); } assert(Condition);}
